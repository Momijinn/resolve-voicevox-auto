...
- Narration UI モードで複数行テキストを入力し、行ごとに Text+ / 音声を再生ヘッドから順次配置
...
- `src/narration_ui.lua` : 複数行入力 UI（分割・並び替え・再生ヘッドから順次配置）
...
1. DaVinci Resolve で実行
   ...
   **最初に必ず `config.lua` を開いて `output_dir` を設定・保存してください。**
   ...
   - ナレーション UI 実行: `narration_ui.lua`（複数行入力して行ごとに Text+ / 音声を生成・順次配置）
