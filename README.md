...
- ワンショットモードで再生ヘッド位置の Text+ クリップだけを音声合成して配置
- Narration UI モードで複数行テキストを入力し、専用の空タイムライン **tmp_narration** に音声＋字幕を生成（完了時に通知）。生成後、目的のタイムラインへ手動でコピー＆ペースト
...
- `src/one_shot.lua` : 再生ヘッド位置の Text+ を1件だけ合成・配置
- `src/watch_start.lua` : 監視実行（疑似リアルタイム）
- `src/watch_stop.lua` : 監視停止
- `src/config.lua` : 設定GUI
- `src/narration_ui.lua` : 複数行入力 UI（専用の空タイムライン `tmp_narration` に音声＋字幕を生成 → 手動コピペ）
...
|---|---|---|
| macOS | 23.3 | ✅ 動作確認済み |
| DaVinci Resolve | 20.3.2 | ✅ 全機能動作確認済み |
| DaVinci Resolve | 21.x | ⚠️ 音声配置・生成は動作。字幕 API（`GetItemListInTrack("subtitle")` 等）がクラッシュするため **Link Subtitle+Audio** は機能しない |
...
1. DaVinci Resolve で実行
   ...
   **最初に必ず `config.lua` を開いて `output_dir` を設定・保存してください。**
   ...
   - ワンショット実行: `one_shot.lua`（再生ヘッド位置の Text+ クリップだけを合成・配置）
   - ナレーション UI 実行: `narration_ui.lua`（複数行入力 → 空タイムライン `tmp_narration` に音声＋字幕を生成 → 通知 → 目的のタイムラインへ手動コピペ）
   - 監視開始: `watch_start.lua`
   - 監視停止: `watch_stop.lua`
...
- **Narration UI は専用の空タイムライン `tmp_narration` に音声＋字幕を生成します**（字幕は空タイムラインでのみ正しい位置に配置できる Resolve API 制約への対応）。生成後に通知が出るので、`tmp_narration` の内容を選択してコピーし、目的のタイムラインへ貼り付けてください。音声は `text_track_index` と同じ番号のオーディオトラックに配置されます。`tmp_narration` は毎回作り直されます（既存の同名タイムラインは削除されます）。
- 対応するクリップは **Text+** のみです。プレーンな「テキスト」ジェネレーターは DaVinci Resolve のスクリプト API 制限によりテキストを読み取れないため非対応です。
- `overwrite = false`（デフォルト）なら変更したセグメントのみ再合成できません。テキストを編集した場合は `overwrite = true` にして `one_shot.lua` を実行するか、`watch_start.lua` を使用してください。
- 監視モードは単一起動で使用してください（ロックファイルで多重起動を防止）。
- `one_shot.lua` / `watch_start.lua` 実行時は VOICEVOX Docker (`voicevox/voicevox_engine:cpu-ubuntu24.04-0.26.0-dev`) をホスト port `50022` で自動起動します。
...
- **Link Subtitle+Audio が動作しない（Resolve 21）**:
  - Resolve 21 では字幕トラック操作 API（`GetItemListInTrack("subtitle")` 等）が SIGABRT でクラッシュする既知の問題がある
  - **Resolve 20.x を使用することを推奨**（20.3.2 で全機能動作確認済み）
  - Resolve 21 でも音声生成・配置・SRT エクスポートは正常に動作する
